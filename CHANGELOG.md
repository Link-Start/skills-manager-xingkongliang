...
- An install checkout is detached from its promisor remote before it leaves this module, so it is not a partial clone. This is what keeps the narrow clone from taxing every future change: callers run plain `git` against the checkout, and without the detach any command reaching an unfetched object would silently become a network round trip that can hang. Detached, a missing object is an immediate error — exactly as in the shallow full checkout that came before — so the network-capable surface stays confined to three commands in one file, and a test fails if that boundary is removed.
